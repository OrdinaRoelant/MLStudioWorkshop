...
Wait a couple of minutes for the run to actually start (check the **Status** on the **Details** tab, it should eventually say *Running, Model training*) and then keep refreshing the Models tab to see the action unfold. When you see the first models loaded in, continue with the *Explore model details* section below.  
